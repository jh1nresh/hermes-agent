...
class AIAgent:
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        return _ct.build_kwargs(
            ...
            model_lower=(self.model or "").lower(),
            base_url_lower=self._base_url_lower,
            ...
            qwen_metadata={"sessionId": self.session_id or "hermes", "promptId": str(uuid.uuid4())} if _is_qwen else None,
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if self.api_mode == "codex_responses":
                        ...
                        # Pre-extract for truncation heuristic (raw SDK message needed)
                        assistant_message = response.choices[0].message
                    ...
                    if hasattr(response, 'usage') and response.usage:
                        ...
                        if self._use_prompt_caching:
                            if self.api_mode == "anthropic_messages":
                                ...
                                _ctc = self._get_chat_completions_transport()
                                _cc_cache = _ctc.extract_cache_stats(response)
                                cached = _cc_cache["cached_tokens"] if _cc_cache else 0
                                written = _cc_cache["creation_tokens"] if _cc_cache else 0
            ...
            try:
                if self.api_mode == "codex_responses":
                    ...
                    _cct = self._get_chat_completions_transport()
                    _cc_nr = _cct.normalize_response(response)
                    assistant_message = SimpleNamespace(
                        content=_cc_nr.content,
                        tool_calls=[
                            SimpleNamespace(
                                id=tc.id,
                                type="function",
                            ...
                            for tc in (_cc_nr.tool_calls or [])
                        ] or None,
                        reasoning=_cc_nr.reasoning,
                        reasoning_content=None,
                        reasoning_details=(
                            _cc_nr.provider_data.get("reasoning_details")
                            if _cc_nr.provider_data else None
                        ),
                    )
                    finish_reason = _cc_nr.finish_reason
