...
这是独立命令。Hermes 的终端认证流程（`hermes acp --setup`）在模型选择后也会将浏览器引导作为后续问题提供，因此大多数用户无需直接运行 `--setup-browser`。
...
Zed 通过 Zed 设置中的自定义 agent 服务器条目支持外部 agent：
...
使用兼容 ACP 的插件，并将其指向本地的 `hermes acp` 命令（参见上面 VS Code / Zed 的 `command`/`args` 示例——形式相同）。
...
Provider 解析使用 Hermes 的正常运行时解析器，因此 ACP 继承当前配置的 provider 和凭据。Hermes 还提供终端认证方法（`--setup`），以便首次运行的客户端打开 Hermes 的交互式模型/provider 设置。
...
- 验证自定义 `agent_servers`（Zed）或等效的编辑器配置是否指向 `hermes acp`。
- Hermes 已安装且在 PATH 中。
...
或编辑 `~/.hermes/.env`。ACP 客户端也可以触发 Hermes 的终端认证流程，该流程运行相同的交互式 provider/模型设置。
